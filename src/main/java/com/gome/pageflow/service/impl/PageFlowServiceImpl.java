...
import com.gome.pageflow.dao.PageFlowDao;
...
import com.gome.pageflowplus.dao.PageFlowPlusDao;

@Service
public class PageFlowServiceImpl implements PageFlowService {

	private Logger logger = Logger.getLogger(PageFlowServiceImpl.class);
	@Autowired
	private PageFlowDao dao;
	@Autowired
	private PageFlowPlusDao plusDao;
	...
	public Map<String, Object> queryTopN(PageFlowQueryParams params) {
		...
		if (urls != null && !urls.isEmpty()) {
			...
			param.put("dataSourceLike", params.getDataSourceLike());
			...
			if (!"pagetype".equals(params.getDim().toLowerCase())) {
				
				List<PageFlow> clickList = null;
				try {
					clickList = dao.queryTopnClick(param);
				} catch (Exception e) {
					logger.error("点击量为空");
				}

				if (clickList != null) {
					for (PageFlow pflow : clickList) {
						clickMap.put(pflow.getPageContent(), pflow.getClick());
					}
				}
			}
	...
	public Map<String, Object> queryChart(PageFlowQueryParams params) {
		...
		paramMap.put("dataSourceLike", params.getDataSourceLike());
		...
		for (PageFlow pageflow : ALlstationChart) {
			...
			// 全站的登入率是100%为了避免kylin访问 计算访问误差所以用登入数代替
			pageflow.setEntriesRate(pageflow.getEntries(), pageflow.getEntries());
		...
		if (!"pagetype".equals(params.getDim().toLowerCase()) && urls.length > 0) {

			List<PageFlow> clickList = dao.queryPageChartClick(paramMap);

			if (clickList != null && !clickList.isEmpty() && clickList.get(0) != null) {
				for (PageFlow pflow : clickList) {
					clickMap.put(pflow.getPageContent() + "_" + pflow.getDate(), pflow.getClick());
				}
			}
		}
	...
	public Map<String, Object> querySearch(PageFlowQueryParams params) {
		...
		if (urls != null && urls.length > 0) {
			...
			if (!"pagetype".equals(params.getDim().toLowerCase())) {
				...
				click = dao.querySearchPageTotalClick(paramMap);
			...
			if (!"pagetype".equals(params.getDim().toLowerCase())) {
				...
				List<PageFlow> clickList = dao.queryTopnClick(paramMap);

				if (clickList != null) {
					for (PageFlow pflow : clickList) {
						String key = pflow.getPageContent();
						clickMap.put(key, pflow.getClick());
					}
				}
			}
			for (PageFlow pageflow : pageflows) {
	...
	public Map<String, Object> querySearchChart(PageFlowQueryParams params) {
		...
		Map<String, Long> clickMap = new HashMap<String, Long>();
		Map<String, Long> allClickMap = new HashMap<String, Long>();

		if (!"pagetype".equals(params.getDim().toLowerCase())) {

			List<PageFlow> allClickList = dao.querySearchClickGroup(paramMap);
			...
			if (clickList != null) {
				for (PageFlow pflow : clickList) {
					...
					String key = pflow.getPageContent() + "_" + pflow.getDate();

					clickMap.put(key, pflow.getClick());
				}
			}

			if (allClickList != null) {
				for (PageFlow pflow : allClickList) {
					String key = pflow.getPageContent() + "_" + pflow.getDate();

					allClickMap.put(key, pflow.getClick());
				}
			}
		...
		for (PageFlow pageflow : pageallStations) {
			...
			String key = pageflow.getPageContent() + "_" + pageflow.getDate();

			long pageClick = allClickMap.get(key) == null ? 0 : allClickMap.get(key);
	...
	public Map<String, Object> queryPlusTopN(PageFlowQueryParams params) {
		...
		if (urls != null && !urls.isEmpty()) {
			...
			if (!"pagetype".equals(params.getDim().toLowerCase())) {
				...
				List<PageFlow> clickList = plusDao.queryTopnClick(param);
	...
	public Map<String, Object> queryPlusSearch(PageFlowQueryParams params) {
		...
		if (urls != null && urls.length > 0) {
			...
			if (!"pagetype".equals(params.getDim().toLowerCase())) {
				...
				try {
					click = plusDao.querySearchPageTotalClick(paramMap);
				} catch (Exception e) {
					logger.error("点击量为空");
				}
			...
			if (!"pagetype".equals(params.getDim().toLowerCase())) {
				...
				List<PageFlow> clickList = plusDao.queryTopnClick(paramMap);
	...
	public Map<String, Object> queryPlusChart(PageFlowQueryParams params) {
		...
		if (!"pagetype".equals(params.getDim().toLowerCase()) && urls.length > 0) {
			...
			List<PageFlow> clickList = plusDao.queryPageChartClick(paramMap);
	...
	public Map<String, Object> queryPlusSearchChart(
		...
		if (!"pagetype".equals(params.getDim().toLowerCase())) {
			...
			List<PageFlow> allClickList = plusDao.querySearchClickGroup(paramMap);
